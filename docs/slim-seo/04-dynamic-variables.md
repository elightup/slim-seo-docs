...
By default, WordPress sets the [meta title](/slim-seo/meta-title-tag/) to "Post title - Site title". But you might have an "event" post type and want to set the meta title to show the address and the date for all events. So when people search for events, they will immediately see the details. Or you might have a "book" post type and want to show the author name of the book in the meta title.

It is not that this cannot be done before. But it cannot be done for a whole series of events, because you have to write by hand for each event or book. It will be very time-consuming!

There is another popular example: review articles. When you write a review, for example, [Slim SEO vs. SEOPress](https://wpslimseo.com/slim-seo-vs-seopress/), it needs to be updated every year. So every time you update it, you should change the year on the title to something like "2024 review". You will need a way to update the year in the title automatically.
...
Go to **Settings > Slim SEO**, you will see a new tab **Meta Tags**, replacing the two old tabs in previous versions, **Homepage** and **Post Types**. This tab allows you to configure meta tags for all types of content, including homepage, post types, taxonomies, and other pages, such as author archives.
...
When you select it, it will be inserted into the input box. You can select or insert multiple variables as you want. You can also type the variable into the input box, or type text between them.

## Using dynamic variables for single posts and terms

When you edit a single post, or a single term (category, tag, or any taxonomy), the plugin also provides you with the same dynamic variables. But there is a big difference: **you can preview the value of the meta tags** as soon as you write it. This is a very important point for you to see how they are rendered as well as their length (and you will see an indicator for whether they are too long or too short).

![Use dynamic variables for single posts with preview](img/dynamic-variables-for-single-posts-with-preview.png)

There is a small note about the preview. For post types or taxonomies in the settings page, the indicator for the number of characters and also the preview may not work. The reason is that the plugin will not be able to know which specific post or term you are configuring for. So, generating a preview will not be possible.
