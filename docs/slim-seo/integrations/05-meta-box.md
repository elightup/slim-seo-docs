...
Slim SEO works with the [Meta Box](https://metabox.io) plugin so you can use your custom fields in meta titles and meta descriptions. When Meta Box is installed and active, the integration is automatically turned on.
...
If your field or meta box ID uses hyphens, the variable will use underscores instead (for example, `my-field` becomes `my_field` in `{{ mb.box.my_field }}`).

:::

**Examples:**

A meta box called "Product Info" with a field "Subtitle" might appear as **[Meta Box] Product Info** with an option like **Subtitle**. In the template you will see something like `{{ mb.product_info.subtitle }}`.
...
Fields that do not store a value (heading, divider, custom HTML, button) or do not have a suitable value (background, fieldset text, text list, sidebar) do not appear in the variable list.
...
- **User**: Value comes from the relevant user (for example, the post author or the author archive being viewed).
