...
| `twitter:image` | Image URL: fallback to the featured image, then the first image in the post content, then the global social image (set in **Settings > Slim SEO > Social**) if no image is set. |
...
The recommended size for the image when sharing on X is **1200x600** pixels.
...
## How to change the X image programmatically?

To change the X image with code, use this snippet:
