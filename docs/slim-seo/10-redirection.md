...
Go to **Settings > Slim SEO** and select the **Redirection** tab, then click the **Add Redirect** button:
...
---|---
Type|Redirection type. See the **Redirection type** section below for details.
From URL|The URL you want to redirect. You can set a rule for an exact URL, or if it contains a text. See the **Matching URLs** section below for details.
To URL|The destination URL that you want to redirect to.
Note|Something that reminds you about this redirection rule.
Enable|Whether this redirect is active or not. If not active, no redirections will be performed.
Ignore parameters|Turn ON if you do not need the requested URL to have the querystring parameters matched with the From URL.
...
- **307 Temporarily Redirect**: it is the HTTP 1.1 successor of the 302 redirect. Major crawlers treat it like a 302 in most cases. It is useful when content is really moved only temporarily (such as during maintenance) AND the server has already been identified by the search engines as 1.1 compatible. Since it is essentially impossible to determine whether the search engines have identified a page as compatible, it is generally best to use a 302 redirect for content that has been temporarily moved.
- **410 Content Deleted**: tells the search engines that the content is gone. If users visit this URL (without redirects), they will see a not found page. It is useful for pages that you deleted on your website.
- **451 Content Unavailable for Legal Reasons**: it is useful when you are ordered by a judge to delete a page or if you get a notice and takedown request. This tells search engines that there was a post here and that you wanted to fulfill this request, but you cannot for legal reasons.
...
All these formats will be converted to a standard format of `path/to/url`. The process of converting a URL to a standard format is called **normalization**. It is used to compare and match the from URL and the requested URL.
...
- **Regex**: the requested URL must match the entered value using a regular expression.

## Export/Import Redirects

You can export your redirects in CSV format by using the **Export** button.

![export redirect](img/redirection-export-import.webp)

You can import your redirects by using the **Import** button.

A new popup with instructions will appear that lets you upload the CSV file.

Please note that your CSV file must have a header row and exactly 7 columns as follows:

Column name|Data type|Description|Possible values|Sample
---|---|---|---|---
...
Note|String|Something to remind you about the redirects||
...
To enable 404 logging, go to the **Settings** tab and select **Enable 404 logs**:

![enable to log 404 pages](https://imgur.elightup.com/s1sjmOf.png)

After clicking the **Save Changes** button, you will see the **404 Logs** tab, where you can see all 404 pages.
...
When a URL is visited many times, it is best to create a redirect for it. To do that, click the **Add Redirect icon** in the **Actions** column.

![add a redirect for a 404 link](https://imgur.elightup.com/PRu4tcZ.png)

A popup will appear that helps you to create a redirect. It is similar to adding a redirect manually, except that the **From URL** is **auto filled by the 404 URL**.

![enter redirect details for a 404 link](https://imgur.elightup.com/kywAzvB.png)

Enter the **To URL** and click the **Add Redirect** button to finish.

### Deleting the 404 log table in the database

When enabling 404 logs, the plugin will create a table in the database to store the 404 URL details. If you do not want to log 404, then you might want to delete this table to keep your database clean. To do that, click the **Delete 404 logs table**:
...
Auto redirection helps you redirect some URLs that have low value to users, or in some cases, are not needed at all. Redirecting them to other URLs helps you remove duplicated content on your website. Slim SEO does this automatically without any settings.

### Attachment pages to files

When you upload any file to WordPress, WordPress creates an attachment page for that file. This page provides very little information about the file and the page looks quite empty. As search engines already index the files, allowing attachment pages to be indexed makes duplicated content and increases the bounce rate. It is better to disable the attachment pages.
...
Setting|Description
---|---
Force trailing slash|Ensures all URLs on your site have the same format, with or without trailing slash (`/`), and redirects the other format to the correct one.
...
Slim SEO provides a filter for developers to skip a redirect. It is useful if you want to programmatically enable or disable a redirect:
