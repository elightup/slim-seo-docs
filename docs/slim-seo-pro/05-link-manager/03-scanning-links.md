...
After installing the plugin, you need to scan all posts to find links and their details. This is required to create reports in the plugin dashboard.

To scan links, click the **Scan links now** button:
...
In that case, we recommend you enable the WordPress cron to run the scanner once, and then disable the WordPress cron when you finish.

## Re-scan links

In case you already ran the link scanning and you want to scan again, go to the **Tools** tab and you will find the **Scan links** button there:

![Run the scan links tool again](https://imgur.elightup.com/mbEry3A.png)

:::caution

The link scanning action will remove all existing link data and scan from the beginning.
...
For the **Shield Security** plugin, go to the plugin settings > Config tab > Traffic, and update the settings **Max Request Limit** and **Request Limit Time Interval**.
...
If you see the scanning process is not finished, follow these steps:

1. Update Slim SEO Pro to the latest version.
2. Deactivate the plugin and reactivate it.
3. Re-scan links.

When you deactivate the plugin, your data remains. No links are lost or changed.

After scanning links, [discover the reports](/slim-seo-pro/link-manager/reports/).
