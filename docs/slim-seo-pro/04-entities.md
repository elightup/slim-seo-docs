...
Entities help search engines and AI assistants understand the **topics and concepts your content covers**. When you tag posts with the right entities, Google gets clearer context about what your content is about. LLMs can cite and recommend it more accurately.
...
Entities are not the same as your site's author or organization profile. That is what the [**Schema**](/slim-seo-pro/schema/adding-schemas/) feature handles (see below). Entities are about the *subjects* your content discusses.
...
**Better AI citations.** LLMs like ChatGPT, Perplexity, and Google's AI Overviews use structured data to decide who and what to cite. Clear entity signals make your content a more reliable source. When someone asks about a topic you cover, the AI is more likely to point to you.
...
**Richer search results.** Structured data can trigger enhanced results in Google, like knowledge panels, FAQ carousels, and sitelinks. These make your listings more eye-catching and clickable.
...
3. Under **Allowed Post Types**, select which post types should show the Entities panel in the editor (for example, Posts, Pages, or custom post types).
4. Save your settings.

Once enabled, you will see an **Entities** tab in the editor for those post types.
...
- **Name** - The entity's name (for example, "Kubernetes", "Tailwind CSS", "Core Web Vitals").
- **URL** - The official page for this entity (for example, the project homepage or Wikipedia page).
- **@id** - A unique identifier (URL or IRI) for this entity in structured data. It helps search engines connect mentions of this entity across your site.
...
- **Type** - The schema.org type. It defaults to `Thing`. Most topic entities work well as `Thing`. You can use more specific types like `SoftwareApplication` or `People` if they fit.
...
**Use both when:** You want the full picture. Schema handles the page-level and identity structure. Entities adds the subject-level context. For example, Schema marks a page as an Article with author info. Entities marks it as being *about* Kubernetes and *mentioning* Docker.
...
**What happens if I delete an entity that is attached to posts?**
